...
const Header = () => {
  ...
  const { itemCount: wishlistCount, openSidebar: openWishlistSidebar } =
    useWishlist();
  const { isAuthenticated, user, backendUser, backendUserLoading, loading } =
      ...
      try {
        ...
        if (data.success && data.data && data.data.length > 0) {
          ...
          const iconMap = {
            ...
            "Trolley Bags": (
              <FaSuitcaseRolling className="w-8 h-8 mb-2 text-rose-500" />
            ),
            "Rolling Duffle": (
              <FaSuitcaseRolling className="w-8 h-8 mb-2 text-rose-500" />
            ),
          ...
          const mappedCategories = data.data.map((cat) => {
            ...
            return {
              ...
              icon: iconMap[cat.name] || (
                <FaShoppingBag className="w-8 h-8 mb-2 text-rose-500" />
              ),
          ...
          <nav className="hidden xl:flex items-center space-x-1">
            ...
            >
              ...
              >
                Categories
                <svg
                  className={`w-4 h-4 ml-1 transition-transform duration-200 ${isCategoryDropdownOpen ? "rotate-180" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {/* Mega Menu Dropdown */}
              <div
                className={`mega-menu fixed left-0 right-0 mt-4 bg-white/90 backdrop-blur-xl shadow-2xl z-50 transition-all duration-300 transform origin-top border-t border-rose-100 ${
                  ...
                  <div className="grid grid-cols-5 gap-6">
                    {megaMenuCategories.map((cat, index) => (
                      <Link
                        to={cat.path}
                        key={index}
                        className="group cursor-pointer p-4 rounded-2xl hover:bg-white hover:shadow-xl transition-all duration-300"
                      ...
                      >
                        ...
                        <div className="text-center">
                          <h3 className="font-bold text-slate-800 group-hover:text-rose-600 transition-colors text-lg">
                            {cat.name}
                          </h3>
              ...
              <form onSubmit={handleSearch} className="relative group">
                <div className="relative">
                  <input
                    ...
                    onFocus={() =>
                      searchQuery.length === 0 && navigate("/search")
                    }
                  />
                  <div
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="p-1">
                    <Link
                      to="/profile"
                      className="flex items-center px-3 py-2 text-sm text-slate-600 hover:bg-rose-50 hover:text-rose-600 rounded-xl transition-colors duration-200 cursor-pointer"
        ...
        {isMobileMenuOpen && (
          <div className="xl:hidden mt-4 pb-6 border-t border-white/10 animate-fade-in-up bg-white/95 backdrop-blur-xl rounded-2xl p-4 shadow-2xl absolute left-4 right-4 top-16">
            ...
            <nav className="flex flex-col space-y-1">
              ...
              <div
                className={`overflow-hidden transition-all duration-300 ${isMobileCategoryOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
              >
                {megaMenuCategories.map((cat, index) => (
                  <Link
                    key={index}
              ...
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className="px-3 py-3 text-slate-700 hover:bg-rose-50 hover:text-rose-600 rounded-xl cursor-pointer font-medium transition-colors"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  {item.name}
                </Link>
              ...
              {isAuthenticated && user ? (
                <>
                  <Link
                    to="/profile"
                    className="px-3 py-3 text-slate-700 hover:bg-rose-50 hover:text-rose-600 rounded-xl flex items-center cursor-pointer font-medium"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                  ...
                  <Link
                    to="/wishlist"
                    className="px-3 py-3 text-slate-700 hover:bg-rose-50 hover:text-rose-600 rounded-xl flex items-center justify-between cursor-pointer font-medium"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    <div className="flex items-center">
                      <FaHeart className="mr-3 opacity-70" /> Wishlist
                    </div>
                    {wishlistCount > 0 && (
                  ...
                  >
                    <div className="flex items-center">
                      <FaShoppingCart className="mr-3 opacity-70" /> Cart
                    </div>
                    {cartItemCount > 0 && (
                      <span className="bg-rose-500 text-white text-xs font-bold rounded-full px-2 py-0.5">
                        {cartItemCount}
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-3 text-rose-600 hover:bg-rose-50 rounded-xl font-medium mt-2"
                  >
                    Logout
                  </button>
                </>
                ...
                <Link
                  to="/login"
                  className="px-3 py-3 bg-gradient-to-r from-rose-600 to-pink-600 text-white rounded-xl text-center font-bold hover:shadow-lg cursor-pointer mt-2 block"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Login / Register
                </Link>
