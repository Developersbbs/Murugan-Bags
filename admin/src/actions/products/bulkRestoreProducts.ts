...
export async function bulkRestoreProducts(
    ...
    try {
        ...
        if (!response.ok) {
            console.error("Bulk product restore failed:", result);
            return {
                dbError: result.error || "Failed to restore products.",
                success: false
            };
        }
        ...
        console.error("Unexpected error in bulkRestoreProducts:", error);
