...
const OrderDetailsModal = ({
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'shipped': return <TruckIcon className="h-6 w-6 text-blue-500" />;
      case 'processing': return <ClockIcon className="h-6 w-6 text-yellow-500" />;
  ...
  // FIX 1: Resolve display name - variant name takes priority
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <span className={"text-lg font-semibold capitalize " + getStatusColor(order.status)}>{order.status}</span>
                    {order.trackingNumber && <span className="text-sm text-gray-600">Tracking: {order.trackingNumber}</span>}
                  </div>
                    ...
                    <div key={item.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <h4 className="font-medium text-gray-900">{getDisplayName(item)}</h4>
                      ...
                      <div className="text-right">
                        ...
                        {order.status === 'delivered' && (
                          <button
                            onClick={() => { setSelectedProductForReview(item); setShowReviewModal(true); }}
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                  <MapPinIcon className="h-5 w-5 mr-2" />Shipping Address
                </h3>
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                  <CreditCardIcon className="h-5 w-5 mr-2" />Payment Information
                </h3>
                <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50">
            <div className="flex space-x-3">
              {order.trackingNumber && (
                <button onClick={() => onTrackOrder(order.trackingNumber)} className="inline-flex items-center px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors">
                  <TruckIcon className="h-4 w-4 mr-2" />Track Order
                </button>
              )}
              <button onClick={() => onDownloadInvoice(order.id)} className="inline-flex items-center px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors">
                <ArrowDownTrayIcon className="h-4 w-4 mr-2" />Download Invoice
              </button>
            </div>
            <button onClick={onClose} className="px-6 py-2 bg-gray-600 text-white font-medium rounded-lg hover:bg-gray-700 transition-colors">Close</button>
          </div>
        </div>
      </div>
      ...
      {showReviewModal && selectedProductForReview && (
        <ReviewModal
          ...
          onClose={() => { setShowReviewModal(false); setSelectedProductForReview(null); }}
