    ...
    if (product.product_variants && product.product_variants.length > 0) {
      ...
      if (isPublishing) {
        ...
        if (availableVariants.length === 0) {
          ...
          const stockInfo = availableVariants
            .map((v) => `${v.name}: ${v.stock}/${v.minStock}`)
            .join(", ");
          return {
            canPublish: true,
        ...
        const stockInfo = product.product_variants
          .map((v) => `${v.name}: ${v.stock}/${v.minStock}`)
          .join(", ");
        return {
          canPublish: true,
...
function transformVariantsForDB(
  variantsData,
  baseSKU,
  costPrice,
  salesPrice,
  ...
  const transformedVariants = variantsData.combinations.map((combo, index) => {
    ...
    if (!sku || variantsData.autoGenerateSKU) {
      ...
      sku = attributeValues
        ? `${baseSKU.toUpperCase()}-${attributeValues}`
        : `${baseSKU.toUpperCase()}-${index}`;
    }
    ...
    const variantCostPrice =
      combo.costPrice ?? combo.cost_price ?? costPrice ?? 0;
    ...
    const variantPublished =
      combo.published !== undefined ? combo.published : true;
  ...
  console.log(
    "🚨 BACKEND: Total transformed variants:",
    transformedVariants.length,
  );
  ...
  try {
    ...
    if (status) {
      ...
      try {
        ...
        /* ignore token parse errors — fall through to published:true */
      }
    ...
    if (color) {
      ...
      if (filter.$or) {
        filter.$and = [{ $or: filter.$or }, { $or: colorQuery }];
        delete filter.$or;
      } else {
    ...
    const productsData = products.map((product) => {
      ...
      if (
        productObj.product_variants &&
        productObj.product_variants.length > 0
      ) {
        productObj.product_variants = productObj.product_variants.map(
          (variant) => {
...
router.get("/slug/:slug", async (req, res) => {
  try {
    ...
    if (!slug || typeof slug !== "string" || slug.trim() === "") {
      return res
        .status(400)
        .json({ success: false, error: "Product slug is required" });
    }
    ...
    const baseStock = await Stock.findOne({
      productId: product._id,
      variantId: null,
    });

    if (baseStock) {
      productObj.baseStock = baseStock.quantity;
      productObj.minStock = baseStock.minStock;
    }

    // Get variant stocks if this is a variant product
    if (productObj.product_variants && productObj.product_variants.length > 0) {
      productObj.product_variants = await Promise.all(
        productObj.product_variants.map(async (variant) => {
          const variantWithImageUrl = variant.images
            ? { ...variant, image_url: variant.images }
            : variant;

          // Fetch variant stock from Stock collection
          const variantStock = await Stock.findOne({
            productId: product._id,
            variantId: variant._id,
          });

          if (variantStock) {
            variantWithImageUrl.stock = variantStock.quantity;
            variantWithImageUrl.minStock = variantStock.minStock;
          }

          if (
            variantWithImageUrl.attributes &&
            variantWithImageUrl.attributes instanceof Map
          ) {
            const attributesObj = {};
            ...
            variantWithImageUrl.attributes &&
            typeof variantWithImageUrl.attributes === "object"
          ) {
            const attributesObj = {};
            Object.entries(variantWithImageUrl.attributes).forEach(
              ([key, value]) => {
                attributesObj[key] = value;
              },
            );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch product by slug" });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    // ✅ FIX: Fetch stock data from Stock collection
    const Stock = require("../models/Stock.js");
...
router.post("/", uploadDigitalFile, async (req, res) => {
  try {
    ...
    if (variantsData && variantsData.combinations) {
      variantsData.combinations.forEach((combo, index) => {
        ...
        if (combo.images && Array.isArray(combo.images)) {
          ...
          const httpUrls = combo.images.filter(
            (img) => typeof img === "string" && img.startsWith("http"),
          );
          if (httpUrls.length > 0) {
            hasFirebaseUrls = true;
            console.log(
              `🚨 BACKEND: Found ${httpUrls.length} Firebase URLs for variant ${index} - skipping variantImagesMap population`,
            );
          }
        }
      });
    ...
    if (!hasFirebaseUrls) {
      console.log(
        "🚨 BACKEND: No Firebase URLs found, populating variantImagesMap from variantsData",
      );
      if (variantsData && variantsData.combinations) {
        variantsData.combinations.forEach((combo, index) => {
          if (combo.images && Array.isArray(combo.images)) {
            // Filter out any non-string values
            const imageUrls = combo.images.filter(
              (img) => typeof img === "string",
            );
            if (imageUrls.length > 0) {
              variantImagesMap[index] = imageUrls;
              console.log(
                `🚨 BACKEND: Added ${imageUrls.length} local URLs to variantImagesMap[${index}]`,
              );
            }
          }
        });
      ...
      console.log(
        "🚨 BACKEND: Firebase URLs found, keeping variantImagesMap empty to prevent duplication",
      );
    }
    ...
    if (req.files && Array.isArray(req.files)) {
      ...
      // Process images
      const imageFields = Object.keys(filesByField).filter(
      ...
      imageFields.forEach((key) => {
        filesByField[key].forEach((file) => {
          allImageUrls.push(
            file.firebaseUrl || `/uploads/products/${file.filename}`,
          );
        });
      });
        ...
        variantImageFields.forEach((key) => {
          ...
          if (match) {
            ...
            if (file) {
              ...
              variantImagesMap[comboIndex].push(
                file.firebaseUrl || `/uploads/products/${file.filename}`,
              );
            }
          }
    ...
    if (allImageUrls.length === 0 && req.body.image_url) {
      try {
        ...
        if (Array.isArray(providedImageUrls) && providedImageUrls.length > 0) {
          ...
          console.log(
            "✅ Using provided image URLs from request body:",
            allImageUrls.length,
          );
        }
      } catch (error) {
    ...
    const productStructure =
      req.body.product_structure || req.body.productStructure || "simple";
    ...
    const productData = {
      ...
      cost_price:
        productStructure === "simple"
          ? parseFloat(req.body.cost_price || req.body.costPrice)
          : undefined,
      selling_price:
        productStructure === "simple"
          ? parseFloat(req.body.selling_price || req.body.salesPrice)
          : undefined,
    ...
    if (req.body.isCodAvailable !== undefined)
      productData.isCodAvailable =
        req.body.isCodAvailable === "true" || req.body.isCodAvailable === true;
    if (req.body.isFreeShipping !== undefined)
      productData.isFreeShipping =
        req.body.isFreeShipping === "true" || req.body.isFreeShipping === true;
    if (req.body.showRatings !== undefined)
      productData.showRatings =
        req.body.showRatings === "true" || req.body.showRatings === true;
    ...
    if (productType === "digital") {
      ...
      const downloadFormat =
        req.body.download_format || req.body.downloadFormat;
    ...
    console.log(
      "product_structure in productData:",
      productData.product_structure,
    );
    ...
    const seoKeywords = req.body.seo_keywords
      ? typeof req.body.seo_keywords === "string"
        ? JSON.parse(req.body.seo_keywords)
    ...
    if (
      ...
      req.body.seo_canonical ||
      req.body.seo_robots ||
      req.body.seo_og_title ||
      req.body.seo_og_description ||
      req.body.seo_og_image
    ) {
      productData.seo = {};
      ...
      if (
        req.body.seo_og_description &&
        req.body.seo_og_description.trim() !== ""
      ) {
    ...
    if (
      ...
      if (req.body.variantImageUrls) {
        try {
          const variantImageUrls =
            typeof req.body.variantImageUrls === "string"
              ? JSON.parse(req.body.variantImageUrls)
              : req.body.variantImageUrls;

          console.log("=== PROCESSING FIREBASE VARIANT IMAGES ===");
          console.log(
            "Variant image URLs from frontend:",
            JSON.stringify(variantImageUrls, null, 2),
          );
          ...
          Object.entries(variantImageUrls).forEach(([index, urls]) => {
            ...
            if (!isNaN(idx) && variantsData.combinations[idx]) {
              ...
              console.log(
                `Added ${newUrls.length} images to variant ${idx}:`,
                variantsData.combinations[idx].images,
              );
            }
          });
      ...
      console.log(
        ...
        JSON.stringify(transformedVariants[0], null, 2),
      );
      ...
      if (transformedVariants.length > 0) {
        ...
        const finalVariants = transformedVariants.map((variant, index) => {
          const finalVariant = {
            ...
            published:
              variant.published !== undefined ? variant.published : true,
    ...
    try {
      ...
      try {
        ...
        staffMember = await Staff.findOne({ is_active: true }).sort({
          created_at: 1,
        });
      } catch (staffError) {
        console.error(
          "Error finding staff member for stock creation:",
          staffError,
        );
      }
      ...
      if (productType === "physical") {
        ...
        if (product.product_structure === "variant") {
          ...
          if (product.product_variants && product.product_variants.length > 0) {
            for (let i = 0; i < product.product_variants.length; i++) {
              ...
              stockEntries.push({
                ...
                minStock:
                  variant.minStock !== undefined ? variant.minStock : null,
                notes: `Initial stock for variant: ${variant.slug}`,
                lastUpdatedBy: staffMember ? staffMember._id : null,
              });
            }
            console.log(
              `✅ Created ${product.product_variants.length} stock entries for variants of product ${product.name}`,
            );
          } else {
          ...
          stockEntries.push({
            ...
            variantId: null,
            quantity:
              product.baseStock !== undefined ? product.baseStock : null,
            minStock: product.minStock !== undefined ? product.minStock : null,
            notes: "Initial stock from product creation",
            lastUpdatedBy: staffMember ? staffMember._id : null,
          });
          console.log(
            `✅ Created stock entry for simple product ${product.name}`,
          );
        }
      }
      ...
      if (stockEntries.length > 0) {
        ...
        console.log(
          `✅ Created ${stockEntries.length} stock entries for product ${product.name}`,
        );
      } else {
    ...
    if (err.code === 11000) {
      ...
      if (field === "sku") {
        ...
        errorMessage =
          "This product name would create a duplicate URL slug. Please choose a different name.";
      } else if (field === "variants.sku") {
        errorMessage =
          "One of the variant SKUs is already in use. Please ensure all variant SKUs are unique.";
      }
    ...
    if (err.message && err.message.includes("Duplicate variant SKUs")) {
      return res.status(400).json({
        success: false,
        error:
          "Duplicate variant SKUs detected within the product. Each variant must have a unique SKU.",
        details:
          process.env.NODE_ENV === "development" ? err.message : undefined,
      });
    }
...
router.put("/:id", uploadDigitalFile, async (req, res) => {
  try {
    ...
    if (req.body.categories) {
      try {
        ...
        console.log(
          ...
          JSON.stringify(parsedCategories, null, 2),
        );
          ...
          for (const cat of parsedCategories) {
            ...
            if (cat.categoryId) {
              if (
                ...
                cat.categoryId.length === 24 &&
                /^[a-fA-F0-9]{24}$/.test(cat.categoryId)
              ) {
              ...
              if (
                ...
                cat.category.length === 24 &&
                /^[a-fA-F0-9]{24}$/.test(cat.category)
              ) {
            ...
            if (categoryId) {
              ...
              if (cat.subcategoryIds && Array.isArray(cat.subcategoryIds)) {
                for (const subcatId of cat.subcategoryIds) {
                  if (
                    typeof subcatId === "string" &&
                    subcatId.length === 24 &&
                    /^[a-fA-F0-9]{24}$/.test(subcatId)
                  ) {
    ...
    const productStructure =
      req.body.productStructure || req.body.product_structure;
    ...
    if (productStructure === "simple") {
      if (req.body.cost_price || req.body.costPrice)
        updateData.cost_price = parseFloat(
          req.body.cost_price || req.body.costPrice,
        );
      if (req.body.selling_price || req.body.salesPrice)
        updateData.selling_price = parseFloat(
    ...
    if (productType === "physical") {
      ...
      if (hasMinStockUpdate)
        updateData.minStock = parseInt(req.body.min_stock_threshold);

      const isSimpleStructure =
        (productStructure || currentProduct?.product_structure) === "simple";
      if (isSimpleStructure && (hasStockUpdate || hasMinStockUpdate)) {
        ...
        if (typeof effectiveBaseStock === "number") {
          if (
            effectiveBaseStock >
            (typeof effectiveMinStock === "number" ? effectiveMinStock : 0)
          ) {
    ...
    if (req.body.isCodAvailable !== undefined)
      updateData.isCodAvailable =
        req.body.isCodAvailable === "true" || req.body.isCodAvailable === true;
    if (req.body.isFreeShipping !== undefined)
      updateData.isFreeShipping =
        req.body.isFreeShipping === "true" || req.body.isFreeShipping === true;
    if (req.body.showRatings !== undefined)
      updateData.showRatings =
        req.body.showRatings === "true" || req.body.showRatings === true;
    ...
    if (productType === "digital") {
      ...
      if (req.body.download_format !== undefined)
        updateData.download_format = req.body.download_format;
      if (req.body.license_type !== undefined)
        updateData.license_type = req.body.license_type;
      if (req.body.download_limit !== undefined)
        updateData.download_limit = parseInt(req.body.download_limit);
    }

    // SEO fields
    if (
      req.body.seo_title !== undefined ||
      req.body.seo_description !== undefined ||
      seoKeywords.length > 0 ||
      req.body.seo_canonical !== undefined ||
      req.body.seo_robots !== undefined ||
      req.body.seo_og_title !== undefined ||
      req.body.seo_og_description !== undefined ||
      req.body.seo_og_image !== undefined
    ) {
      updateData.seo = {};
      ...
      if (transformedVariants.length > 0) {
        ...
        const finalVariants = transformedVariants.map((variant, index) => {
          ...
          if (currentProduct.product_variants) {
            const match = currentProduct.product_variants.find(
              (v) =>
    ...
    if (req.body.published !== undefined && productStructure !== "variant") {
      ...
      if (isPublishing !== currentPublished) {
        ...
        const productForValidation = {
          ...
          ...updateData,
          product_variants:
            updateData.product_variants || currentProduct.product_variants,
        ...
        console.log(
          "Product structure:",
          productForValidation.product_structure,
        );
        console.log("Product type:", productForValidation.product_type);
        console.log(
          "Variants count:",
          productForValidation.product_variants?.length || 0,
        );
        if (
          productForValidation.product_variants &&
          productForValidation.product_variants.length > 0
        ) {
        ...
        const stockValidation = validateStockForPublish(
        ...
        if (!stockValidation.canPublish) {
          return res.status(400).json({
            ...
            isZeroStock:
              (stockValidation.stockInfo?.baseStock ??
                currentProduct.baseStock ??
                0) === 0,
          });
        }
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Error finding staff member for stock update:",
          staffError,
        );
      }
      ...
      if (
        product.product_structure === "simple" &&
        (product.baseStock !== undefined || product.minStock !== undefined)
      ) {
        const stockUpdate = {
      ...
      if (product.product_variants && product.product_variants.length > 0) {
        ...
        const currentVariantIds = product.product_variants.map((v) =>
          v._id.toString(),
        );
        ...
        console.log(
          "Existing stock entries count:",
          existingStockEntries.length,
        );

        // Delete stock entries for variants that no longer exist
        const stockEntriesToDelete = existingStockEntries.filter(
          (stockEntry) =>
            !currentVariantIds.includes(stockEntry.variantId.toString()),
        );
        ...
        if (stockEntriesToDelete.length > 0) {
          ...
          console.log(
            `🗑️ Deleted ${stockEntriesToDelete.length} orphaned stock entries`,
          );
        }
        ...
        for (let i = 0; i < product.product_variants.length; i++) {
          ...
          if (variant.stock !== undefined || variant.minStock !== undefined) {
            ...
            if (existingEntry) {
              ...
              console.log(
                `✅ Updated existing stock entry for variant: ${variant.slug}`,
              );
            } else {
              // Create new entry
              await Stock.create(stockUpdate);
              console.log(
                `✅ Created new stock entry for variant: ${variant.slug}`,
              );
            }
          }
        }
  ...
  try {
    ...
    for (const product of productsToDelete) {
      ...
      for (const imageFileName of mainImageFileNames) {
        ...
        console.log(
          `Bulk delete - Attempting to delete main product image: ${imageFileName}`,
        );
        ...
        if (fs.existsSync(imagePath)) {
          ...
          console.log(
            `⚠️ Bulk main product image file not found: ${imageFileName}`,
          );
        }
      }
      ...
      if (product.product_variants && product.product_variants.length > 0) {
        for (let i = 0; i < product.product_variants.length; i++) {
          ...
          if (variant.images && variant.images.length > 0) {
            console.log(
              `Bulk delete - Variant ${i} images to delete:`,
              variant.images,
            );
            ...
            for (const imageUrl of variant.images) {
              if (imageUrl) {
                ...
                console.log(
                  `Bulk delete - Attempting to delete variant image: ${imageFileName}`,
                );
                console.log(`Bulk delete - Full path: ${imagePath}`);
                console.log(
                  `Bulk delete - File exists: ${fs.existsSync(imagePath)}`,
                );
                ...
                if (fs.existsSync(imagePath)) {
                  try {
                    fs.unlinkSync(imagePath);
                    console.log(
                      "✅ Bulk deleted variant image:",
                      imageFileName,
                    );
                  } catch (deleteError) {
                    console.error(
                      `❌ Bulk failed to delete variant image ${imageFileName}:`,
                      deleteError.message,
                    );
                  }
                } else {
                  console.log(
                    `⚠️ Bulk variant image file not found: ${imageFileName}`,
                  );
                }
              } else {
    ...
    try {
      ...
      console.log(
        `✅ Bulk deleted ${deletedStocks.deletedCount} stock entries for ${productsToDelete.length} products`,
      );
    } catch (stockError) {
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (mainImageFileNames.length > 0) {
      try {
        for (const imageFileName of mainImageFileNames) {
          const imagePath = path.join(uploadDir, imageFileName);
          console.log(
            `Attempting to delete main product image: ${imageFileName}`,
          );
          ...
          if (fs.existsSync(imagePath)) {
            ...
            console.log(
              `⚠️ Main product image file not found: ${imageFileName}`,
            );
          }
        }
      } catch (storageError) {
        ...
        if (variant.images && variant.images.length > 0) {
          ...
          for (const imageUrl of variant.images) {
            if (imageUrl) {
              ...
              console.log(
                `Attempting to delete variant image: ${imageFileName}`,
              );
              console.log(`Full path: ${imagePath}`);
              console.log(`File exists: ${fs.existsSync(imagePath)}`);
              ...
              if (fs.existsSync(imagePath)) {
                try {
                  ...
                  console.error(
                    `❌ Failed to delete variant image ${imageFileName}:`,
                    deleteError.message,
                  );
                }
              } else {
                console.log(
                  `⚠️ Variant image file not found: ${imageFileName}`,
                );
              }
            } else {
    ...
    try {
      ...
      console.log(
        `✅ Deleted ${deletedStocks.deletedCount} stock entries for product ${product.name}`,
      );
    } catch (stockError) {
...
router.patch("/toggle-status", async (req, res) => {
  try {
    ...
    if (currentProduct.product_structure === "variant" && variantId) {
      ...
      const hasPublishedVariants = currentProduct.product_variants.some(
        (v) => v.published,
      );
      const hasStock = currentProduct.product_variants.some(
        (v) => v.published && v.status === "selling",
      );
    ...
    if (published) {
      // When publishing, determine status based on stock
      const baseStock =
        typeof currentProduct.baseStock === "number"
          ? currentProduct.baseStock
          : 0;
      const minStock =
...
router.patch("/toggle-archive-status", async (req, res) => {
  try {
    ...
    console.log(
      `PATCH /toggle-archive-status called. ID: ${id}, VariantID: ${variantId}, Archived: ${archived}`,
    );
  ...
  try {
    ...
    const csvData = products.map((product) => {
      ...
      const subcategories =
        product.categories
          ?.flatMap(
            (c) => c.subcategories?.map((s) => s.name).filter(Boolean) || [],
          )
          .join("; ") || "";
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to export products as CSV" });
  }
});
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          try {
            ...
            const productData = {
              ...
              slug:
                row["Slug"] ||
                row["Product Name"].toLowerCase().replace(/\s+/g, "-"),
              sku: row["SKU"],
