...
const HomePage = () => {
  ...
  const [comboOffers, setComboOffers] = useState([
    { _id: '1', title: "Couple Travel Pack", description: "2 Premium Backpacks + 1 Duffle Bag", price: 7999, originalPrice: 12999, isLimitedTime: true, savingsPercent: 38 },
    { _id: '2', title: "Family Pack", description: "2 School Bags + 2 Laptop Bags", price: 5999, originalPrice: 9999, isLimitedTime: true, savingsPercent: 40 }
  ]);
  ...
  useEffect(() => {
    const fetchComboOffers = async () => {
      try {
        const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
        const res = await fetch(`${API_URL}/combo-offers`);
        const data = await res.json();
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {comboOffers.map((combo, index) => (
              <div
                key={combo._id || index}
                className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl p-10 border border-slate-700 hover:border-rose-500/50 transition-all duration-500 transform hover:scale-[1.02] hover:shadow-2xl relative overflow-hidden group"
                style={{
